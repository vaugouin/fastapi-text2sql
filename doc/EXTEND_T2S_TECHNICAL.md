...
|---|---|---|
| Aspect ratio (per movie) | `T_WC_T2S_MOVIE.ASPECT_RATIO varchar(20)`, indexed | [doc/sql/T2S-tables.sql:653](sql/T2S-tables.sql#L653), [:703](sql/T2S-tables.sql#L703) |
| Color flag | `T_WC_T2S_MOVIE.IS_COLOR int(11)`, indexed | [doc/sql/T2S-tables.sql:647](sql/T2S-tables.sql#L647), [:698](sql/T2S-tables.sql#L698) |
| Black & white flag | `T_WC_T2S_MOVIE.IS_BLACK_AND_WHITE int(11)`, indexed | [doc/sql/T2S-tables.sql:648](sql/T2S-tables.sql#L648), [:699](sql/T2S-tables.sql#L699) |
| Silent flag | `T_WC_T2S_MOVIE.IS_SILENT int(11)`, indexed | [doc/sql/T2S-tables.sql:649](sql/T2S-tables.sql#L649), [:700](sql/T2S-tables.sql#L700) |
| 3D flag | `T_WC_T2S_MOVIE.IS_3D int(11)`, indexed | [doc/sql/T2S-tables.sql:650](sql/T2S-tables.sql#L650), [:701](sql/T2S-tables.sql#L701) |
| Technical reference table | `T_WC_T2S_TECHNICAL(ID_TECHNICAL, ID_RECORD, ID_WIKIDATA, DESCRIPTION, DESCRIPTION_FR, OVERVIEW, TECHNICAL_TYPE, …, MOVIE_COUNT, SERIE_COUNT, WIKIPEDIA_IMAGE_PATH, IMDB_RATING, IMDB_RATING_WEIGHTED, POPULARITY)` — 56 active rows | [doc/sql/T2S-tables.sql:1987-2027](sql/T2S-tables.sql#L1987-L2027); standalone dump at [doc/sql/T_WC_T2S_TECHNICAL.sql](sql/T_WC_T2S_TECHNICAL.sql) |
| Junction | `T_WC_T2S_MOVIE_TECHNICAL(ID_MOVIE, ID_TECHNICAL, …)` | [doc/sql/T2S-tables.sql:1013-1034](sql/T2S-tables.sql#L1013-L1034) |
...
|---|---|---|
| Schema | Drop `ASPECT_RATIO` column + index; insert ~12 rows in `T_WC_T2S_TECHNICAL`; backfill rows in `T_WC_T2S_MOVIE_TECHNICAL` | [doc/sql/T2S-tables.sql:653](sql/T2S-tables.sql#L653), [:703](sql/T2S-tables.sql#L703), [:1987](sql/T2S-tables.sql#L1987) |
...
| Stage | Change | File:line |
|---|---|---|
| Schema | Insert 3 rows in `T_WC_T2S_TECHNICAL`; backfill `T_WC_T2S_MOVIE_TECHNICAL` from flags; columns + indexes preserved. | [doc/sql/T2S-tables.sql:1987](sql/T2S-tables.sql#L1987) |
...
1. **`IS_3D` → `3d_movie` technical row** ([doc/sql/T2S-tables.sql:650](sql/T2S-tables.sql#L650)). Direct analog of Proposal B. Same B.1 / B.2 tradeoff applies; same recommendation (keep the flag, add the technical row). High value, low risk.
...
- `COLOR_TECHNOLOGY varchar(100)` ([doc/sql/T2S-tables.sql:651](sql/T2S-tables.sql#L651))
- `FILM_TECHNOLOGY mediumtext` ([:652](sql/T2S-tables.sql#L652))
- `FILM_FORMAT varchar(50)` ([:654](sql/T2S-tables.sql#L654))
- `SOUND_SYSTEM mediumtext` ([:655](sql/T2S-tables.sql#L655))
- `SOUND_TECHNOLOGY varchar(200)` ([:656](sql/T2S-tables.sql#L656))
...
| PR-2 | Insert color_movie / black_and_white_movie / silent_movie rows; backfill junction from flags; add `Technical_format` aliases | DB migration + [data/closed_vocabularies.json](../data/closed_vocabularies.json); no prompt change, no code change | Low — new rows appear in metadata collection only | ✅ **Schema + aliases shipped 2026-05-20** (rows 57-59 in [T_WC_T2S_TECHNICAL.sql](sql/T_WC_T2S_TECHNICAL.sql)); junction backfill pending (§12.5) |
