...
**All five knobs reach the run signature (FASTAPI-TEXT2SQL-234, 2026-09-25).**
`T_WC_T2S_EVALUATION_EXECUTION` has `RESULT_ENTITY_MODEL` and `ANSWER_SINGLE_VALUE_MODEL`
since `migrate-evaluation-execution-late-models.sql`. The skip rule, the scoring (phase 20)
and the export (phase 32) are keyed on all five, and a run that moves one of those two lands
in its own folder, suffixed from the ROW: `<version>_<lang>_<ee>_<t2s>_<complex>_re-<model>`
and/or `_asv-<model>`. Rows written before the columns existed hold NULL, which is matched as
the default `gpt-4o` and never backfilled, so every folder written so far keeps its name and
the analysis scripts that build the three-model path keep working. Their exports label the
two models `null`, meaning "unknown, written before the column", not `gpt-4o`;
`api_output.llm_model_result_entity` still gives what the API actually used.
...
**This is not SQL, and the resemblance is the trap.** The table above is the whole language: anything else is either rejected or, worse, quietly misread. Written 2026-09-09, after two assertions were authored with `AWARD_NAME NOT LIKE '%Academy Awards'` and caught before they reached the bank.

- **`LIKE`, `BETWEEN`, `IS NULL`, a bare `NOT`, `%` wildcards: none exist.** They match neither the `IN` branch (no standalone `IN`) nor the comparison pattern (no recognised operator), so they land in `_evaluate_comparison_assertion` and return *"Invalid comparison syntax"*. The assertion then scores 0 **whatever the data does**, which is worse than carrying no assertion at all: it looks like a failing evaluation, and nothing in the score column distinguishes the two. Use `NOT IN ('exact value')` instead, naming the intruders rather than describing them by pattern.
- **`<>` is the dangerous one, because it raises nothing.** The comparison pattern `(\w+)\s*(==|!=|<=|>=|<|>)\s*(.+)` matches the `<` and takes `> 'x'` as the expected value, so the assertion is silently mis-evaluated rather than reported. Write `!=`.

A sweep of the 866 result assertions in the bank on 2026-09-09 found **zero** occurrences of any of the six, through the evaluator itself and by text search. The language is respected today; this note exists to keep it that way.
...
- `RESOLUTION_MODE`, `RESOLUTION_MODE_RESPECTED`, `COMPLEX_MODEL_USED`, `ANSWER_SINGLE_VALUE_PROCESSING_TIME` — the escalation verdict (FASTAPI-TEXT2SQL-257). `RESOLUTION_MODE` is the mode the bank declared **at the time of that run**, copied rather than joined so that re-qualifying a question later does not rewrite the meaning of campaigns already played. `RESOLUTION_MODE_RESPECTED` is 1/0, and `NULL` when the mode is `any` or undeclared. **It is deliberately kept out of `ASSERTIONS_TOTAL_SCORE`**: that score is what makes one campaign comparable to the previous one, and folding a new failure reason into it would make 1.1.19 incomparable with 1.1.18. `COMPLEX_MODEL_USED` exists because `COMPLEX_QUESTION_PROCESSING_TIME` misses a quarter of the escalations (46 against 34 on campaign `001.001.018`), the gap being the direct scalar answer, which banks into its own timer
- `FIRST_PASS_FAILURE_CODE`, `QUERY_MODE` : the escalation **cause**, where the four columns above give only its existence (FASTAPI-TEXT2SQL-271). `FIRST_PASS_FAILURE_CODE` mirrors the response field of the same name, a closed vocabulary: `descriptive_identification`, `requires_complex_resolution`, `unbacked_entity_literal`, `text2sql_error`, `sql_guard_rejected`, `sql_execution_error`, `entity_fallback_unmatchable`, `no_results:<signal>[+<signal>]`. Those causes do not carry the same verdict, which is why lumping them was expensive: a question routed on purpose because the extraction classified it `descriptive_identification` is the dispositif working, while a `requires_complex_resolution` is Text2SQL admitting defeat, and the two used to share one label. **`NULL` with `COMPLEX_MODEL_USED = 1` is itself a cause**, the direct scalar answer, the one path that escalates without going through the retry. `QUERY_MODE` is read from `first_pass_entity_extraction` when present and from `entity_extraction` otherwise, never the other way round: on a retry the second holds the inner extraction of the **rewritten** question and reads `named_entity_query`, so reading it first would count nearly every descriptive routing as a named one (measured 2026-09-17 on 505 local logs: 30 first-pass `descriptive_identification`, of which 27 show `named_entity_query` in `entity_extraction`). It is also the only denominator available: `FIRST_PASS_FAILURE_CODE` exists only on retried rows, so a rate of "descriptive questions that escalated" needs `QUERY_MODE` on every row
- `ENTITY_EXTRACTION_MODEL`, `TEXT2SQL_MODEL`, `COMPLEX_MODEL` — the tuple that disambiguates executions. Since FASTAPI-TEXT2SQL-234 they are joined by `RESULT_ENTITY_MODEL` and `ANSWER_SINGLE_VALUE_MODEL`: the five together disambiguate executions. The two late columns are NULL on rows written before 2026-09-25, and NULL is matched as the default `gpt-4o`
- `LLM_PROMPT_TOKENS`, `LLM_CACHED_TOKENS`, `LLM_COMPLETION_TOKENS`, `LLM_REASONING_TOKENS` — the request's token totals, all tasks (FASTAPI-TEXT2SQL-296, `migrate-evaluation-execution-llm-tokens.sql`). Computed in phase 20 from the `llm_usage` block of `JSON_RESULT`, so a rescore fills them for any row whose API returned it; NULL on older rows, meaning **not measured**, never zero. Prompt includes cached, completion includes reasoning, as OpenAI bills. The split per task stays in `JSON_RESULT` (`JSON_EXTRACT(JSON_RESULT, '$.llm_usage.text2sql.reasoning_tokens')`), and phase 20 prints it as a table: uncached in, cached in, visible out, reasoning, per task
...
| [bench-result-entity.py](bench-result-entity.py) | Off-production A/B bench for the **answer-entity classifier** (`f_classify_result_entity`), sibling of the row above. Same discipline: two models over the same questions in one process, no API server, no execution row, no cache write. `uv run eval/bench-result-entity.py [--model-a M] [--model-b M] [--lang en|fr] [--limit N] [--workers N] [--run VERSION] [--min-decidable N] [--noise-floor N] [--out FILE]`. **Why it exists rather than a pass-rate comparison:** the classifier is authoritative over the text-to-SQL model's own `result_entity` but only acts on disagreement, so it is inert on most of the bank; an evaluator campaign would spend $86 watching a task do nothing (since FASTAPI-TEXT2SQL-234 such a campaign would at least land in its own folder, but it would still measure a task that does nothing on most questions). **Ground truth is free and needs no database:** an execution that passed its assertions had the right answer type, so its `result_entity` is a label. Run `001.001.018` yields **689 EN** and **643 FR** usable questions after dropping the ones that failed, the ones labelled outside the classifier's 19-word vocabulary (`movie_video`, `movie_serie`, which come from the text-to-SQL model), and duplicates. **Three outcomes, never one accuracy number:** *correct*; *abstained* (`""` or an unknown word, which falls back to the pre-existing behaviour and costs nothing); *confidently wrong* (a different valid label, the only outcome that overrides a query that may have been right). The verdict is built on the confident-error rate alone. **Read the per-class table, not the headline:** `movie`+`person`+`serie` carry 90 % of the labels, so answering "movie" every time already scores ~54 %; the bench prints that majority baseline beside every score, and refuses a percentage for any class under `--min-decidable` (default 30) rather than printing one computed on seven examples. `--limit` samples **round-robin across classes**, rarest first, so a 14-question smoke run touches 14 classes instead of fourteen movies |
...
| [text2sql-eval.sh](text2sql-eval.sh) | Build image + run container (detached, host network). **Asks before spending anything** (2026-09-23): it builds the image, runs a read-only pre-flight inside it and prints the target colour and the version it answers, `bktrees_ready`, the language(s), the model of each of the five tasks, every option, how many calls are left per language after the skip rule, the rows phase 10 will hard-delete, an estimated wall-clock and cost, drift between `~/docker/text2sql-eval` and the git clone, and every scheduled task under `~/docker` that is still armed, read from the `mv` lines of each repository's `off.sh` (tmdb-movie-preprocess, wikipedia-crawler, wikidata-crawler, movieparadise, sqlite, embedding-update and the others; `letsencrypt` is kept on). Stopping a container is not enough, cron relaunches it at its next tick. **One command each way** (2026-09-26): `bash ~/docker/off-all.sh` runs every `off.sh` before the campaign, `bash ~/docker/on-all.sh` every `on.sh` after it; the script says so at startup, recommends them in the pre-flight (falling back to the per-folder loop if either is missing, `OFF_ALL`/`ON_ALL` override the paths), and repeats the `on-all.sh` reminder at launch, since the run is detached and nothing else will remind you when it ends. Order: `off-all.sh` first, then `docker stop` for the containers still running, which `off.sh` does not stop; `on-all.sh` once the last campaign of the session has ended, not between the English and French halves. Only a typed `yes` launches the run; with no terminal attached it aborts unless `EVAL_CONFIRM=yes` is set. **Exception, the rescore-only run** (2026-09-25): when the pre-flight counts zero API calls left and zero bank rows to translate, no LLM token can be spent, and the script launches without asking. That is the run that follows an assertion correction: phase 10 purges the soft-deleted executions, phase 20 rescores, phases 30-32 re-export. Everything is overridable from the environment: `API_VERSION`, `EVAL_LANGUAGE` (not `LANGUAGE`, which is the gettext locale variable and may hold `en_US:en`), `ENTITY_EXTRACTION_MODEL`, `TEXT2SQL_MODEL`, `COMPLEX_MODEL`, `RESULT_ENTITY_MODEL`, `ANSWER_SINGLE_VALUE_MODEL`, `STORE_TO_CACHE`, `COMPLEX_MODEL_USED`. The last two model variables were added with FASTAPI-TEXT2SQL-232; since FASTAPI-TEXT2SQL-234 each has its own column, and a run that moves either one is announced as landing in its own folder (`_re-<model>`, `_asv-<model>`). **The defaults are the baseline run** (2026-09-14): `API_VERSION=1.1.19` and gpt-4o on all five tasks, so a bare `./text2sql-eval.sh` scores the current pipeline against nothing but itself. That run is the prerequisite to any model comparison, because the prompts moved substantially after the `001.001.018` reference and its pass rates no longer describe the code. Example of the comparison that follows it: `TEXT2SQL_MODEL=gpt-5.6-terra API_VERSION=1.1.19 ./text2sql-eval.sh` |
...
| [new-evaluations-person-role-collapse.sql](new-evaluations-person-role-collapse.sql) | One-shot batch (FASTAPI-TEXT2SQL-211), NOT YET APPLIED: four bilingual evaluations for the case where a question designates its answer by a ROLE while naming a different person as a property of the work ("the costume designer of Capote with Philip Seymour Hoffman"). The bank's 15 existing role-plus-person questions all answer with MOVIES, so none of them could express the collapse, let alone catch it. Assertions carry no ID on purpose: `COUNT(*) &gt; 0` catches the defect head-on, and `PERSON_NAME NOT IN ('...')` states the invariant that ages well, the answer is never the person named in the question. Includes the counter-example ("who both directed and acted in Pulp Fiction?"), because one human in two roles is legitimate and must keep answering |
...
|---|---|---|
| 2.8 | 2026-09-09 | **The assertion DSL is not SQL, and §4.3 now says so where it will be read.** Two assertions written for FASTAPI-TEXT2SQL-238 used `AWARD_NAME NOT LIKE '%Academy Awards'`. `LIKE` is not in the language: the expression matches neither the `IN` branch nor the comparison pattern, falls through to `_evaluate_comparison_assertion` and returns *Invalid comparison syntax*, so both would have scored 0 with the fixed prompt exactly as with the broken one, in red, indistinguishable from a real failure. Caught before they reached the bank; replaced with the counter-example form `NOT IN ('exact value')` of §4.5 and verified against the evaluator on hand-built DataFrames. §4.3 gains the list of SQL-isms that do not exist (`LIKE`, `BETWEEN`, `IS NULL`, bare `NOT`, `%`) plus `<>`, which is the dangerous one because the comparison pattern reads its `<` and takes `> 'x'` as the expected value, mis-evaluating in silence rather than reporting. A sweep of all 866 result assertions, through the evaluator and by text search, found zero occurrences of any of the six. |
