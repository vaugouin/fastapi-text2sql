...
0a. **Vision pre-stage (only when `image_ref` is supplied)**
   ...
   - **A question about the people in the picture is answered by the faces**, not by the work's cast: "who are the actors on this picture?" on a two-person still returns those two, where it used to return the film's thirty-two credited names. When no face was read, it falls back to the cast. A question naming a role ("who directed this?") is never answered this way, the director not being in the frame.
