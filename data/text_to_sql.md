...
- If the question asks for "movies" or "films" (film, films, movie, movies, "les films", "un film"), add `T_WC_T2S_MOVIE.IS_MOVIE = 1`. A documentary is not a movie: `IS_MOVIE` and `IS_DOCUMENTARY` are exclusive, so without this filter documentaries leak into a list of movies. This holds whatever else qualifies the movies: a person ("movies with Humphrey Bogart", "films de Truffaut", "movies directed by Sergio Leone", "movies starring X"), a technical format, a genre, a year, a country, a company, a topic, a list, an award. The person, the format and the rest are filters on top of `IS_MOVIE = 1`, never a replacement for it.
  Example: "Movies shot in color with Humphrey Bogart" → `SELECT DISTINCT T_WC_T2S_MOVIE.ID_MOVIE, ... FROM T_WC_T2S_MOVIE JOIN T_WC_T2S_MOVIE_TECHNICAL ON T_WC_T2S_MOVIE.ID_MOVIE = T_WC_T2S_MOVIE_TECHNICAL.ID_MOVIE JOIN T_WC_T2S_PERSON_MOVIE ON T_WC_T2S_MOVIE.ID_MOVIE = T_WC_T2S_PERSON_MOVIE.ID_MOVIE JOIN T_WC_T2S_PERSON ON T_WC_T2S_PERSON_MOVIE.ID_PERSON = T_WC_T2S_PERSON.ID_PERSON WHERE T_WC_T2S_MOVIE.IS_MOVIE = 1 AND T_WC_T2S_MOVIE_TECHNICAL.ID_TECHNICAL = '{{Technical_format1}}' AND T_WC_T2S_PERSON.PERSON_NAME = '{{Person_name1}}' AND T_WC_T2S_PERSON_MOVIE.CREDIT_TYPE = 'cast' AND (T_WC_T2S_MOVIE.IS_DOCUMENTARY = 1 OR T_WC_T2S_PERSON_MOVIE.CAST_CHARACTER NOT IN ('Self', 'Himself', 'Herself', '(archive footage)', 'Self (archive footage)', 'Self (archive footage) (uncredited)', 'Self (uncredited)')) ORDER BY T_WC_T2S_MOVIE.IMDB_RATING_WEIGHTED DESC`
...
- Example: "Les films tournés en franscope" → `WHERE T_WC_T2S_MOVIE.IS_MOVIE = 1 AND T_WC_T2S_MOVIE_TECHNICAL.ID_TECHNICAL = '{{Technical_format1}}'`
- Example: "Movies in Technicolor" → `... JOIN T_WC_T2S_MOVIE_TECHNICAL mt ON mt.ID_MOVIE = m.ID_MOVIE WHERE m.IS_MOVIE = 1 AND mt.ID_TECHNICAL = '{{Technical_format1}}'`
- Example (aspect ratio): "Movies in 1.85" / "Films au format 4:3" / "Widescreen movies" → `... JOIN T_WC_T2S_MOVIE_TECHNICAL mt ON mt.ID_MOVIE = m.ID_MOVIE WHERE m.IS_MOVIE = 1 AND mt.ID_TECHNICAL = '{{Technical_format1}}'`. A movie can have several aspect ratios; always go through the junction, never filter on a column of `T_WC_T2S_MOVIE` for aspect ratio.
...
- "movies directed by / written by `<person>`", "films de `<person>`" → `result_entity = movie`; the person is a **crew filter**, and the word "movies" / "films" adds `T_WC_T2S_MOVIE.IS_MOVIE = 1`.
...
- **Every `CREDIT_TYPE = 'cast'` filter on `T_WC_T2S_PERSON_MOVIE` carries the self-appearance exclusion**, whichever direction the question runs: the cast of a movie ("actors in X"), the movies of a person ("movies with Humphrey Bogart"), or a named actor used as a condition inside an `EXISTS`. A person credited as themselves or through archive footage did not act in that film. Documentaries are exempt, since appearing as oneself is what a documentary credit means, so the exclusion is always written as this exact condition, joined with `AND`, on the same credit instance that carries `CREDIT_TYPE = 'cast'` (replace `T_WC_T2S_PERSON_MOVIE` with its alias when the instance is aliased):
  `AND (T_WC_T2S_MOVIE.IS_DOCUMENTARY = 1 OR T_WC_T2S_PERSON_MOVIE.CAST_CHARACTER NOT IN ('Self', 'Himself', 'Herself', '(archive footage)', 'Self (archive footage)', 'Self (archive footage) (uncredited)', 'Self (uncredited)'))`
  It requires `T_WC_T2S_MOVIE` in the query; join it if the query does not already. Series credits (`T_WC_T2S_PERSON_SERIE`) take no exclusion.
...
- **Actor / actress / cast queries (no `{{Department_nameN}}` placeholder)**: when the question is about actors, actresses, or the cast of a movie or serie (e.g. "actors in The Big Lebowski", "actresses born in 1962", "cast of Inception"), filter credits with `CREDIT_TYPE = 'cast'` (NOT `'crew'`) on the appropriate join table:
  - Movies: `T_WC_T2S_PERSON_MOVIE.CREDIT_TYPE = 'cast' AND (T_WC_T2S_MOVIE.IS_DOCUMENTARY = 1 OR T_WC_T2S_PERSON_MOVIE.CAST_CHARACTER NOT IN ('Self', 'Himself', 'Herself', '(archive footage)', 'Self (archive footage)', 'Self (archive footage) (uncredited)', 'Self (uncredited)'))`
  ...
  - The movie line above already carries the `CAST_CHARACTER` self-appearance exclusion; never drop it, including when the actor is a filter rather than the answer ("movies with Humphrey Bogart"). The series line carries none.
...
- **The rule covers every way a named person qualifies a work**: with / avec, starring, by / de, directed by, written by, produced by, composed by. Only the credit filter inside the `EXISTS` changes: `CREDIT_TYPE = 'cast'` plus the self-appearance exclusion for with / starring, `CREDIT_TYPE = 'crew'` plus the matching `CREW_DEPARTMENT` for the others.
- **It applies in reverse too.** When the ANSWER is the film and two persons qualify it ("the film with A directed by B"), each person gets its own `EXISTS` on `T_WC_T2S_MOVIE.ID_MOVIE`, never a shared credit instance, and "film" / "movie" adds `T_WC_T2S_MOVIE.IS_MOVIE = 1`.
- **The mirror case: ONE person in TWO roles on the SAME work.** "Who both directed and acted in Pulp Fiction?", "qui a réalisé et joué dans ce film ?", "actors who directed themselves". Here the two credits do belong to the same human, so they share `ID_PERSON`, but they are still **two rows** of the credit table and still need **two aliased instances, both correlated on the SAME `ID_MOVIE`**:
  `SELECT DISTINCT T_WC_T2S_PERSON.ID_PERSON, T_WC_T2S_PERSON.PERSON_NAME, T_WC_T2S_PERSON.POPULARITY, T_WC_T2S_PERSON.KNOWN_FOR_DEPARTMENT, T_WC_T2S_PERSON.BIRTH_YEAR, T_WC_T2S_PERSON.DEATH_YEAR, T_WC_T2S_PERSON.PROFILE_PATH FROM T_WC_T2S_PERSON JOIN T_WC_T2S_PERSON_MOVIE pm_crew ON pm_crew.ID_PERSON = T_WC_T2S_PERSON.ID_PERSON JOIN T_WC_T2S_PERSON_MOVIE pm_cast ON pm_cast.ID_PERSON = T_WC_T2S_PERSON.ID_PERSON AND pm_cast.ID_MOVIE = pm_crew.ID_MOVIE JOIN T_WC_T2S_MOVIE ON T_WC_T2S_MOVIE.ID_MOVIE = pm_crew.ID_MOVIE WHERE T_WC_T2S_MOVIE.MOVIE_TITLE = '{{Movie_title1}}' AND pm_crew.CREDIT_TYPE = 'crew' AND pm_crew.CREW_DEPARTMENT = 'Directing' AND pm_cast.CREDIT_TYPE = 'cast' AND (T_WC_T2S_MOVIE.IS_DOCUMENTARY = 1 OR pm_cast.CAST_CHARACTER NOT IN ('Self', 'Himself', 'Herself', '(archive footage)', 'Self (archive footage)', 'Self (archive footage) (uncredited)', 'Self (uncredited)')) ORDER BY T_WC_T2S_PERSON.POPULARITY DESC`
  - **"and" between two roles is a CONJUNCTION, never a disjunction.** Writing the two credit filters as `((CREDIT_TYPE = 'crew' AND CREW_DEPARTMENT = 'Directing') OR (CREDIT_TYPE = 'cast'))` on a **single** instance answers "either role" and returns the director plus the whole cast. Measured on 2026-08-26: 37 rows instead of 1.
  - **Both instances must carry the film.** Correlating only one of them means "directed THIS film and acted in ANYTHING", which returns the right answer whenever the person happens to hold any credit of the second kind and the wrong one otherwise. `pm_cast.ID_MOVIE = pm_crew.ID_MOVIE` in the join is what makes the two roles belong to the same work.
  - Same shape on the series side with `T_WC_T2S_PERSON_SERIE` and `ID_SERIE`, and for any pair of roles (cast plus Writing, Directing plus Editing), only the two credit filters changing.
- **Self-check before emitting, two questions.** First: if the question asks for a person by ROLE, no predicate anywhere in the query may force the projected `ID_PERSON` to equal a person NAMED in the question. If one does, you collapsed two people into one and the query returns zero rows whatever the data holds. Second: if the question joins two roles with "and" / "et", count your credit instances. One instance carrying an `OR` is the same error read the other way round, and it returns far too many rows instead of none.
